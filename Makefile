...
ZIPFILE := hangman$(RELEASE).zip
SRCZIPFILE := hangman$(RELEASE)src.zip
BUZIPFILE := hangman$(shell date "+%Y%m%d").zip
...
release: clean all
	...
	$(RM) ../$(SRCZIPFILE)
	$(ZIP) $(SRCZIPFLAGS) ../$(SRCZIPFILE) $(OUTDIR) $(SRCDIR) $(MANUAL) $(MENUDIR) Makefile
